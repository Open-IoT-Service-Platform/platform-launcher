...
var test = function(userToken, accountId, deviceId, deviceToken, cbManager) {
  ...
  return {
    ...
    "sendDataToSingleDevice": function(done) {
      ...
      var index = 0;
      var promchain = Array.apply(0, Array(scaleSamplesNumber)).reduce(async (acc, item) => {
        await acc;
        var valueList = [];
    ...
    "countSingleDeviceData": function(done) {
      ...
      promtests.searchDataAdvanced(BASE_TIMESTAMP + newDeviceNumber * 100, -1, userToken, accountId, deviceIds, componentIds[0], false, undefined, undefined, true)
        .then((result) => {
