    ...
    return {
        "sendAggregatedDataPoints": function(done) {
            ...
            promtests.addComponent(componentNames[0], componentTypes[0], userToken, accountId, deviceId)
                ...
                .then(() => promtests.addComponent(componentNames[1], componentTypes[1], userToken, accountId, deviceId))
                .then((id) => {
                    componentId[1] = id;
                })
                .then(() => promtests.addComponent(componentNames[2], componentTypes[2], userToken, accountId, deviceId))
                .then((id) => {
                    componentId[2] = id;
                })
                .then(() => promtests.addComponent(componentNames[3], componentTypes[3], userToken, accountId, deviceId))
                .then((id) => {
                    componentId[3] = id;
                })
        ...
        "receiveAggregations": function(done) {
            ...
            var allAggregation = [aggr1, aggr2, aggr3, aggr4].reduce(function(acc, val) {
                [0, 1].forEach(function(index) {
                    ...
                    if (val[index][aggregation.COUNT] && acc[index][aggregation.MIN] > val[index][aggregation.MIN]) {
