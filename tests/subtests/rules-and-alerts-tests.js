    ...
    return {
        ...
        "deleteRuleAndSendDataAgain": function(done){
            promtests.deleteRule(userToken1, accountId, rules[switchOnCmdName].id)
                ...
                .then(() => promtests.checkObservations(temperatureValues2, rules[switchOnCmdName].cid,
                    cbManager1, deviceToken, accountId, deviceId, componentParamName, 1000))
                .then(() => {done();})
                .catch((err) => {done(err);});
        },
        "createRulesAndCheckAlarmReset": function(done) {
            promtests.createSimpleRule(rules[switchOffCmdName], userToken1, accountId, deviceId, "Manual")
                ...
                }).then(() => promtests.checkObservations(temperatureValues3, rules[switchOnCmdName].cid,
                    cbManager1, deviceToken, accountId, deviceId, componentParamName, 1000))
                .then(() => { done(); })
                .catch((err) => { done(err); });
        },
        ...
        "cleanup": function(done) {
            ...
            // delete multiple actuation test resources
                ...
                .then(() => helpers.control.deleteComplexCommand(switchOffCmdName1_2, userToken1, accountId))
